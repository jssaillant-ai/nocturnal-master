...
app.use(express.urlencoded({ extended: true, limit: '50mb', verify: (req, res, buf) => { req.rawBody = buf; } }));
...
function verifyGumroadSignature(req) {
    if (!GUMROAD_WEBHOOK_SECRET) {
        console.warn('⚠️ GUMROAD_WEBHOOK_SECRET is not set. Webhook signature not verified.');
        return true; // allow in dev, but should be set in prod
    }
    const signature = req.get('X-Gumroad-Signature');
    if (!signature || !req.rawBody) return false;
    const hmac = crypto.createHmac('sha256', GUMROAD_WEBHOOK_SECRET).update(req.rawBody).digest('hex');
    return timingSafeEqual(signature, hmac);
}

async function upsertUserCreditsByEmail(email, creditsToAdd) {
    const users = await readJSON(USERS_FILE);
    let user = users.find(u => u.userId === email);
    if (!user) {
        user = { userId: email, credits: 0, createdAt: new Date().toISOString() };
        users.push(user);
    }
...
async function processGumroadWebhook(webhookData) {
    ...
    if (priceNum >= 10 || (product_name && product_name.includes('ULTRA'))) {
        tier = 'ULTRA';
        credits = 250;
    } else if (priceNum >= 5 || (product_name && product_name.includes('PRO'))) {
        tier = 'PRO';
        credits = 100;
    } else {
        tier = 'BASIC';
...
app.post('/api/gumroad/webhook', async (req, res) => {
    try {
        ...
        if (GUMROAD_SELLER_ID && req.body.seller_id && req.body.seller_id !== GUMROAD_SELLER_ID) {
            console.error('❌ Invalid seller_id:', req.body.seller_id);
            return res.status(403).json({ error: 'Invalid seller_id' });
        }
...
// Client verification endpoint (used by HTML)
app.post('/verify', async (req, res) => {
    try {
        const { license_key, email } = req.body || {};
        if (!license_key || !email) {
            return res.json({ valid: false, message: 'license_key and email required' });
        }

        const codes = await readJSON(CODES_FILE);
        const codeData = codes.find(c => c.code === String(license_key).toUpperCase());
        if (!codeData) {
            return res.json({ valid: false, message: 'Invalid code' });
        }

        // If code already used by another email, block
        if (codeData.used && codeData.usedBy && codeData.usedBy !== email) {
        ...
        if (!codeData.used) {
            ...
            const totalCredits = await upsertUserCreditsByEmail(email, codeData.credits);

            const analytics = await readJSON(ANALYTICS_FILE);
            analytics.totalActivations = (analytics.totalActivations || 0) + 1;
            analytics.totalCreditsDistributed = (analytics.totalCreditsDistributed || 0) + codeData.credits;
            await writeJSON(ANALYTICS_FILE, analytics);

            return res.json({ valid: true, tokens: totalCredits, addedCredits: codeData.credits, tier: codeData.tier });
        }

        // Already used by same email → return current credits
        const users = await readJSON(USERS_FILE);
        const user = users.find(u => u.userId === email);
        const tokens = user ? user.credits : codeData.credits;
        return res.json({ valid: true, tokens, tier: codeData.tier });
